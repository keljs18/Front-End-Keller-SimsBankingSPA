...
function Card(props) {
  ...
  function validate(field, label) {
    ...
    function validateDeposit(){
        return name.length > 0 && deposit.length > 0;
    }
    function validateWithdrawl(){
        return name.length > 0 && withdraw.length > 0;
    }
    ...
    function handleDeposit() {
        ...
        if (name === users[currentUserIndex].name) {
          ...
          return;
            ...
            setStatus(`No ${label} entered; please complete all fields`);
        ...
        <div className={classes()} style={{maxWidth: "25rem"}}>
            ...
            <div className="card-body">
                ...
                {props.handle && show ? (
                   ...
                   <button type="submit" disabled={!validateForm()} className="btn btn-light" onClick={handleCreate} >Create New Account</button>
                   </>
                ) : (
                props.submitButton && (
                   <>
                ...
                {props.deposit && show ? (
                  <>
                  {userName}'s Balance ${balance} <br/><br/>
                  Account Name<br/>
                  <input type="input" className="form-control" id="name" placeholder="Enter Name" value={name} onChange={e => setName(e.currentTarget.value)} /><br/>
                  ...
                  <button type="submit" disabled={!validateDeposit()}className="btn btn-light" onClick={handleDeposit} >Deposit</button>
                  </>
                ) : (props.submitButtonDeposit && (
                    <>
                    <div className="card text-white text-center bg-success mb-3">
                      <h5>Successfully Deposited ${deposit} into your account. Current Balance ${balance}</h5>
                    </div>
                ...
                {props.withdraw && show ? (
                  ...
                  <button type="submit" disabled={!validateWithdrawl()} className="btn btn-light" onClick={handleWithdraw} >Withdrawl</button>
                  </>
                ) : (props.submitButtonWithdraw && (
                    <>
                    <div className="card text-white text-center bg-success mb-3">
                      <h5>Successfully Withdrew ${withdraw} from your account. Current Balance ${balance}</h5>
                    </div>
                    <button type="submit" className="btn btn-warning" onClick={clearForm} >Make another Transaction</button>
                    </>
                    )
                ...
                {props.login && show ? (
                  ...
                  <button type="submit" disabled={!validateLogin()} className="btn btn-light" onClick={handleLogin} >Log In</button>
                  </>
